...
function tonav() {
  ...
  document.getElementById("page-name").innerText = document.title.split(" | 何小藤")[0];
}
  ...
  (function ($) {
    $.fn.nekoScroll = function (option) {
      var defaultSetting = {
        ...
        borderRadius: 5 + 'px',
        right: 55.6 + 'px',
        nekoImg: "https://resources.hexiaoteng.top/img/laxian.png",
        hoverMsg: "您来啦！",
...
function createtime1() {
  var grt = new Date("12/19/2022 00:00:00"); //此处修改你的建站时间或者网站上线时间
...
function createtime2() {
  ...
  setTimeout(
    console.warn.bind(
      console,
      "%c ⚡ Powered by 何小藤 %c 你正在访问何小藤の小家",
...
function share_() {
  ...
  try {
    ...
    navigator.clipboard.writeText('何小藤的站内分享\n标题：' + subTitle + '\n链接：' + url + '\n欢迎访问！🎉🎉🎉');
    new Vue({
      data: function () {
        this.$notify({
          title: "分享链接复制成功！",
          message: "赶快粘贴发送给您的小伙伴吧！",
          position: 'top-left',
          offset: 50,
          showClose: true,
          type: "success",
...
function createtime() {
  ...
  // 网站诞生时间
  var grt = new Date("12/19/2022 00:00:00");
  ...
  (currentTimeHtml =
    hnum < 18 && hnum >= 9
      ...
      // : `<img class='boardsign' src='https://lskypro.acozycotage.net/Fomalhaut/badge/F小屋-下班休息啦.svg' title='下班了就该开开心心地玩耍~'><br> <div style="font-size:13px;font-weight:bold">本站居然运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'></i> <br> 旅行者 1 号当前距离地球 ${dis} 千米，约为 ${unit} 个天文单位 🚀</div>`),
        ? `<div style="font-size:13px;font-weight:bold">本站已运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'</div>`
        : `<div style="font-size:13px;font-weight:bold">本站已运行了 ${dnum} 天 ${hnum} 小时 ${mnum} 分 ${snum} 秒 <i id="heartbeat" class='fas fa-heartbeat'</div>`),
...
if (localStorage.getItem("blogbg") != undefined) {
  ...
  document.getElementById("defineBg").innerText = `:root{
    --default-bg: url(https://resources.hexiaoteng.top/img/defaultbg.jpg);
    --darkmode-bg:url(https://resources.hexiaoteng.top/img/darkbg.jpg);
    --mobileday-bg: url(https://resources.hexiaoteng.top/img/mobiledefaultbg.jpg);
    --mobilenight-bg: url(https://resources.hexiaoteng.top/img/mobiledarkbg.jpg);
  }`;
}
// 切换背景主函数
function changeBg(s) {
...
<br>
<center><p><button onclick="reset()" style="background:linear-gradient(to right, #f0a7a7, #d0d7de);display:block;width:40%;padding:15px 0;border-radius:30px;color:white;font-size:1.1em;"><i class="fa-solid fa-arrows-rotate"></i>&nbsp;恢复默认设置</button></p></center>
...
<div class="content" style="display:flex">
  ...
  <div class="content-text" style="font-weight:bold; padding-left:10px"> 雪花特效 (浅色模式) </div><input type="checkbox" id="snowSet" onclick="setSnow()">
</div>
